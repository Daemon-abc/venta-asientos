...
# --- 2. CARGA DE DATOS AL PRINCIPIO (Para máxima velocidad) ---
try:
    # Descargamos todo una sola vez por recarga
...
if not df.empty:
    df['Fila'] = pd.to_numeric(df['Fila'], errors='coerce').fillna(0).astype(int)
    df['Asiento'] = pd.to_numeric(df['Asiento'], errors='coerce').fillna(0).astype(int)

# --- 3. CONTROL DE NAVEGACIÓN RÁPIDA ---
...
# Procesar clic desde el mapa visual SIN hacer consultas extras a internet
...
if "sel_id" in query_params:
    ...
    if not df.empty:
        # ⚡ OPTIMIZACIÓN: Buscamos en la memoria local en lugar de ir a Supabase
...
if st.session_state.pantalla == "mapa":
    ...
    st.caption("Toca un asiento para editar.")

    if not df.empty:
        # Estilos CSS: gap reducido a 2px, butacas de 23px y texto de 13px sin padding vacio
        html_mapa = """
        <style>
            ...
            .asiento-link {
                ...
                width: 19px !important;          /* ⬅️ Edita el ANCHO aquí */
                height: 19px !important;         /* ⬅️ Edita el ALTO aquí */
                border-radius: 3px;
                font-weight: bold; 
                font-size: 11px !important;      /* ⬅️ Baja la letra para que quepa bien */
                color: white !important;
                text-decoration: none !important; border: none; text-align: center;
                padding: 0 !important; margin: 0 !important; 
                line-height: 19px !important;    /* ⬅️ Modifica esto igual al ALTO para centrar el número */
        ...
        for zona in zonas:
            ...
            #  NUEVA LÍNEA (Centrada al medio):
           #  NUEVA LÍNEA (Compatible con modo claro y modo oscuro):
...
# =========================================================================
# PANTALLA 2: EL FORMULARIO DE EDICIÓN
# =========================================================================
